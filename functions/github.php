...
function postToGithub($data, $now, $mastodon_uri, $twitter_slug) {
    ...
    // Determine the Slug
    $file_name = $now->format("Y-m-d") . "-" . slugify($title) . ".md";
    ...
    if ($data->category === "bookmark") {
        ...
        if (isset($data->content)) {
            ...
            $yaml_lede = "\nlede: \"" . trim(convert_content(str_replace('"', '\"', $data->content))) . "\"";
        }
    }
    else {
    ...
    $url = Config::$site_url . $data->category . "/" . slugify($title) . "/";
    $github_api_url = "https://api.github.com/repos/" . Config::$github["username"] . "/" . Config::$github["repository"] . "/contents/" . ($status === "draft" ? "_drafts" : "_posts/" . $data->directory) . "/" . $file_name;
    ...
    if (!$GITHUB_response) {
        ...
        create_webmention_data($now, $content_and_links, $url);
...
function updateOnGithub($url, $content) {
