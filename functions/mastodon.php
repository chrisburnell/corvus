...
function postToMastodon($data, $now, $post_to_twitter, $post_to_github) {
    $headers = [
    ...
    if ($post_to_twitter) {
        postToTwitter($data, $now, $output_status["uri"], $post_to_github);
    }
    else if ($post_to_github) {
        postToGithub($data, $now, $output_status["uri"], null);
    }
}
